...
-record(state, {
		target_resource_types = [],		%% 需求列表
		local_resource_tuples = [],		%% 供给列表
		found_resource_typles = []		%% 探测到的资源实例%
	}).
...
handle_cast({add_local_resource, {Type, Instance}}, State) ->
	...
	NewResourceTuples = add_resource(Type, Instance, ResourceTyples),
...
handle_cast(trade_resources, State) ->
	...
	lists:foreach(
		fun(Node) ->
			gen_server:cast({?MODULE, Node}, {trade_resources, {node(), ResourceTuples}})
		end,
		AllNodes),
	{noreply, State};
...
handle_cast({trade_resources, {ReplyTo, Remotes}}, 
			#state{local_resource_tuples = Locals,
					...
					found_resource_typles = OldFound} = State) ->
